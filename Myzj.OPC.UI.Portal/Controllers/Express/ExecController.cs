    ...
    public class ExecController : Controller
    {
        ...
        public JsonResult QuerySubscribeByNo(ExpressSubscribeRecordModel model)
        {
            ...
            if (result.Any())
            {
                result.ForEach(item =>
                    {
                        item.FormatDate = item.RowCreateDate.ToString();
                    });
            }
            return Json(result);
		...
		//public JsonResult QueryTemaiProvider(string orderCode,int orderType)
		//{
		//	var result = new BaseResponse()
		//	{
		//		DoFlag = false,
		...
		//	try
		//	{
		//		var val = ExpressClient.Instance.QueryTemaiProvider(orderCode,orderType);
		//		result.DoFlag = true;
		//		result.DoResult = val;
		//	}
		//	catch (Exception ex)
		//	{
		//		result.DoResult = "查询失败";
		//	}
		//	return Json(result, JsonRequestBehavior.AllowGet);
		//}
    }
